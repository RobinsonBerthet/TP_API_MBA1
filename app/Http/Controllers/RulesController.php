...
class RulesController extends Controller
{
    ...
    public function changeRules(Request $request)
    {
        ...
        // Vérifier que l'utilisateur est un admin (table role) role_id = 1
        if ($user->role_id != 1) {
            return response()->json(['error' => 'Vous n\'avez pas les droits pour accéder à cette ressource'], 403);
        }

        // Ajoute ou modifie dans la table droits une nouvelle ligne avec l'id de la fonctionnalité et le role_id, vérifier en amont que la fonctionnalité n'a pas de droits déjà, sinon la modifier si la ligne est déjà présente
        try {
            $fonctionnalite = Droit::where('fonctionnalite_id', $request->fonctionnalite_id)->first();

            if (!$fonctionnalite) {
                // Création dans la table droits
                $droit = new Droit();
                ...
                // Mise à jour dans la table droits ou réponse droit déjà existant
                $droit = Droit::where('fonctionnalite_id', $request->fonctionnalite_id)
